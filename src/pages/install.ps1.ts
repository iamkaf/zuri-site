...
const apiUrlForRef = (ref: string) =>
  `https://api.github.com/repos/${REPO}/contents/install.ps1?ref=${encodeURIComponent(ref)}`;

export const GET: APIRoute = async ({ url }) => {
  const ref = url.searchParams.get('ref') || DEFAULT_REF;
  const upstream = await fetch(apiUrlForRef(ref), {
    headers: {
      'User-Agent': 'zuri-site-install-proxy',
